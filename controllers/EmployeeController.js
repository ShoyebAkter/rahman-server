      ...
      if (emp.PaymentInits.length > 0) {
        const inits = emp.PaymentInits.map((init) => {
          if (init.Payments.length > 0) {
            const paymentAmounts = init.Payments.map((payment) =>
...
const add = async (req, res) => {
  ...
  try {
    ...
    try {
        console.log("Entry passport",newEmp.id)
      ...
      console.log('Passport created successfully:',passportInfo);
      console.log("Entry visa")
    ...
    //   await transaction.commit();
      ...
      if (error.errors) {
        const error_message = error.errors.map(
          (err) => `For ${err.value} :` + err.message
        );
...
const getExpirePassports = async (req, res) => {
  try {
    ...
    if (passports.length > 0) {
      const expiryPassports = passports
        .map((passport) => {
        ...
        .filter(
          (entry) => entry.daysUntilExpiry <= 90 && entry.daysUntilExpiry > 0
        );
...
const uploadFile = async (req, res) => {
  try {
    ...
    if (fileSizeInBytes > maxSizeInBytes) {
      return res
        .status(500)
        .json({ error: "File size exceeds the maximum allowed size (1 MB)" });
    }
...
const addVisa = async (req, res) => {
  try {
    const { employeeId, viId, referenceNo, status, issueDate, expiryDate } =
      req.body;
